...
class FuzzyMatchingPreparationTests(unittest.TestCase):
    ...
    @patch("src.services.documents.DocumentRepo.actualizar_status")
    ...
    def test_reads_ner_json_and_changes_status(
        self, leer_uno: Mock, leer: Mock, actualizar_status: Mock
    ) -> None:
        doc = SimpleNamespace(status="ner", ner_path="s3/archivosNER/doc.json")
        entities = [
            {
                "text": "Madre de Dios",
                "category": "LOC",
        ...
        actualizar_status.assert_called_once_with(self.db, doc, "fuzzyMatching")
    ...
    def test_does_not_change_status_for_invalid_json(
        ...
        actualizar_status.assert_not_called()
